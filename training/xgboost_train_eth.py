...
def load_data() -> pd.DataFrame:
    ...
    orig = len(df)
    df = df.dropna(subset=["eth_price_at_news", "eth_price_15m", "eth_price_1h"])
    ...
    df["eth_change_15m"]   = (df["eth_price_15m"] - df["eth_price_at_news"]) / df["eth_price_at_news"] * 100
    df["abs_change_15m"]   = df["eth_change_15m"].abs()
    df["is_impactful_15m"] = (df["abs_change_15m"] > THRESHOLD_15M).astype(int)
    df["direction_15m"]    = (df["eth_change_15m"] > 0).astype(int)

    df["eth_change_1h"]   = (df["eth_price_1h"] - df["eth_price_at_news"]) / df["eth_price_at_news"] * 100
    df["abs_change_1h"]   = df["eth_change_1h"].abs()
    df["is_impactful_1h"] = (df["abs_change_1h"] > THRESHOLD_1H).astype(int)
    df["direction_1h"]    = (df["eth_change_1h"] > 0).astype(int)
...
def compute_price_context(df: pd.DataFrame) -> np.ndarray:
    changes = pd.Series(df["eth_change_15m"].values.astype(np.float32))
    shifted = changes.shift(1)
    eth_vol = shifted.rolling(window=20, min_periods=2).std().fillna(0).values.astype(np.float32)
    eth_mom = shifted.rolling(window=5,  min_periods=1).mean().fillna(0).values.astype(np.float32)
    fg      = fetch_fear_greed_index(df["published"])
    return np.column_stack([eth_vol, eth_mom, fg]).astype(np.float32)
...
def main():
    ...
    clf15_path = str(HERE / f"xgb_impact_clf_15m_eth_bert_{variant}.json")
    clf1h_path = str(HERE / f"xgb_impact_clf_1h_eth_bert_{variant}.json")
    reg15_path = str(HERE / f"xgb_price_reg_15m_eth_bert_{variant}.json")
    scaler_path = str(HERE / f"xgb_feature_scaler_eth_bert_{variant}.pkl")
    results_path = HERE / f"xgb_eth_bert_{variant}_results.json"
    ...
    preds_path = str(HERE / f"xgb_eth_bert_{variant}_test_preds.npz")
    np.savez(
        preds_path,
