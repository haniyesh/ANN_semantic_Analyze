...
The system listens to crypto news Telegram channels in real time, passes each headline through a multi-model NLP pipeline, predicts whether it will move BTC price within 15 minutes, and pushes scored results to a live dashboard. One-hour outcomes remain research data but are not used by the live signal.
...
| RAG / historical context | 10 | 1 | Production BERT uses Qdrant RAG; Groq table remains the non-RAG research variant |
| **Total** | **1,578** | **1,562** | |
...
|---|---|---|---|
| **XGBoost + CryptoBERT + RAG (production)** | n\_estimators=500 | 1,578 | CryptoBERT + FinBERT + RoBERTa-Twitter ensemble + Qdrant RAG |
| **XGBoost + Groq/Llama** | n\_estimators=500 | 1,562 | Llama-3.3-70B via Groq API |
| **ANN + CryptoBERT** | 54,615 | 1,569 | Non-RAG research variant |
...
|---|---|---|---|---|---|---|
| **XGBoost + CryptoBERT + RAG (production)** | 0.51 | **76.9%** | **42.3%** | 65.0% | **51.3%** | 80.3% |
| XGBoost + CryptoBERT (non-RAG) | 0.57 | 76.0% | 41.2% | **65.8%** | 50.6% | **80.7%** |
...
For Docker deployment, run both the API and bot services:
...
- `xgb_feature_scaler_bert_rag.pkl` — deployed 1,578-feature scaler
- `xgb_impact_clf_15m_bert_rag.json` — deployed 15-minute RAG classifier
- `ann_bert_weights.pt`, `ann_groq_weights.pt` — ANN weights

A Qdrant instance (`QDRANT_URL` + `QDRANT_API_KEY`) is required for production RAG retrieval. `--skip-rag` produces a separately named non-RAG research bundle and cannot overwrite the deployed RAG artifacts.
