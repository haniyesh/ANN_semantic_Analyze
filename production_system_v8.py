...
RESULTS_PATH  = HERE / "production_results_v8.json"
...
def build_features(
    ...
    type_indices, type_probs = crypto_news_type_classify(emb, return_probs=True)
    ...
    print(f"  Semantic: {semantic.shape[1]} dims (CryptoBERT 768 + sent{len(sent_cols)} + type11)")
...
class EmbProjection(nn.Module):
    def __init__(self, in_dim: int = 768, out_dim: int = EMB_PROJ_DIMS):
        super().__init__()
...
def main():
    print("=" * 65)
    print("  PRODUCTION MULTI-TOWER FUSION — v5 + Cross-Attention + Macro Injection")
    ...
    print(f"  CryptoImpactNetV5 (sentiment-focused) + CrossAttention + NewsTypeGating: {n_params:,} params | params/row: {ratio:.2f} {sign}")
    print(f"    Semantic    : {sem_dim} → (768→{EMB_PROJ_DIMS}) → 12")
    print(f"    NewsTypeGate: type_probs(11) → gate(12) → gated sem(12)")
    print(f"    RAG         : {rag_dim} (Qdrant macro-reweighted) → 6")
    print(f"    Macro       : {macro_dim} (5 features) → 6  [injected into sem before attn]")
    print(f"    s_ctx       : gated_sem(12) + macro(6) = 18  [time-aware query]")
    print(f"    CrossAttn   : s_ctx(18) ↔ rag(6) → 24")
